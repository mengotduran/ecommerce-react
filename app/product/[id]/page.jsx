...
const ProductDetail = dynamic(() => import('../../../components/ProductDetail/ProductDetail'), {
  ssr: false,
  loading: () => (
    <div style={{ minHeight: '100vh', background: 'var(--background)', padding: '32px 32px 0', maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: 48, flexWrap: 'wrap' }}>
        <div style={{ flex: '1 1 380px', display: 'flex', gap: 12 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {[0,1,2].map((i) => <div key={i} className="skeleton" style={{ width: 64, height: 64, borderRadius: 10 }} />)}
          </div>
          <div className="skeleton" style={{ flex: 1, height: 420, borderRadius: 16 }} />
        </div>
        <div style={{ flex: '1 1 300px', border: '1px solid var(--border-color)', borderRadius: 16, padding: 28, display: 'flex', flexDirection: 'column', gap: 14 }}>
